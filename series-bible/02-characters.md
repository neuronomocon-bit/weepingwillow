...
- **WILLOW — DRAFTED CANON (Ch22). THE PROCEDURE, FROM INSIDE THE SUBJECT.** Hand flat on the chest through the shirt, no pressure. **Not a search but a sequence**, worked in a fixed order that has been used before, stopping at each address for an unequal period. **On failing to return it goes back to the top and runs the whole list again**, slower. **Eleven minutes in total, two full passes, nothing removed.** One address holds it for **two minutes and forty seconds** and it does not resolve and cannot leave, *"and when it finally moved it did not stop holding the shape."* **It is holding it now.**
